    ...
    <script>
        jQuery(document).ready(function() {
            jQuery('#pdf_upload').submit(function(event) {
                ...
                jQuery.ajax({
                    url: "<?php if ($subdomain != 'dijivu') { echo substr_replace(get_bloginfo('url'), $subdomain.'.', strpos(get_bloginfo('url'), '/') + 2, 0); }else echo get_bloginfo('url'); ?>/gallery-create-publication-2-ajax/",
                    data: formData,
                    type: 'POST',
