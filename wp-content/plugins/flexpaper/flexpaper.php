...
function plugin_parse_request($wp) {
    ...
    if (array_key_exists('flexpaper-plugin', $wp->query_vars) 
		...
		include( plugin_dir_path( __FILE__ ) . 'mudraw_php5.php');
		...
		if(strlen(get_option('flexpaper_mudraw_installdir'))>0){
            $path_to_mudraw = get_option('flexpaper_mudraw_installdir');
        }
		...
		if(!is_dir($pdf_dir)){
			...
			if(!$validatedConfig){
				...
				if($format == "swf" || $format == "png" || $format == "pdf" || $format == "jpg" || $format == "jpgpageslice"){
					...
					// PNG format
					if($format == "png"){
					    if(mudrawEnabled($path_to_mudraw . $mudraw_exec)){
					        $mudraw=new mudrawpdf();
					        $mudraw->swf_dir = $swf_dir;
                            $mudraw->pdf_dir = $pdf_dir;
                            $mudraw->draw($path_to_mudraw,$pdfdoc,$swfdoc,$page);
					    }else{
...
if(is_admin()){
	...
	$flex_opts = array(
		...
		pdf_base_dir=>substr($pdf_dir,strrpos($pdf_dir,'wp-content')-1),
		...
		progressiveloading=>(get_option('flexpaper_progressive')!=null)?get_option('flexpaper_progressive'):"true",
...
<form method="post" action="options.php">
    ...
    <tr>
    	<td valign="top" style="padding-bottom:0px">
			<table class="form-table">
		        <tr valign="top">
		        <th scope="row">CSS Width <span style="color:#999999;font-style:italic;font-size:12px;">(e.g. 100%)</span></th>
		        <td><input type="text" name="flexpaper_width" value="<?php echo get_option('flexpaper_width'); ?>" /></td>
		        </tr>
		         
		        <tr valign="top">
		        <th scope="row">CSS Height <span style="color:#999999;font-style:italic;font-size:12px;">(e.g. 100%)</span></th>
		        <td><input type="text" name="flexpaper_height" value="<?php echo get_option('flexpaper_height'); ?>" /></td>
		        </tr>
		
		        <tr valign="top">
		        <th scope="row">Initial Scale <span style="color:#999999;font-style:italic;font-size:12px;">(1.3 for 130%)</span></th>
    ...
    <tr>
		...
		<table class="form-table" style="background-color:#EEEEEE;margin-top:0px;">
		<?php
					$path_to_pdf2swf 	= '';
					$path_to_pdf2json 	= '';
					$pdf2swf_exec		= '';
					...
					$mupdf_installed = mudrawEnabled($path_to_mudraw . $mudraw_exec);
			        ...
			        <tr valign="top">
				        <th scope="row">PDF storage directory</th>
				        <td><input style="width:400px;" type="text" name="flexpaper_pdfdirectory" value="<?php echo $pdf_dir ?>" /> <?php if($pdfdir_exists){ ?><img class="conversionParameter" src="../wp-content/plugins/flexpaper/tick.png"><?php }else{ ?><img src="../wp-content/plugins/flexpaper/icon_no.png"> This directory does not exist. Please specify a new directory path. <?php } ?></td>
			        </tr>

                    <tr valign="top" style="padding-bottom:50px;">
	                ...
	                <tr>
	                    <td colspan="2" style="padding-left:0px;padding-top:60px;"><span style="font-size:15px;"><b>On the Fly Publishing - Conversion Settings</b></span><br/>FlexPaper supports all major browsers without any further configuration but if you would like to support older browsers and devices then consider using on the fly publishing.</td>
	                </tr>
			        ...
			        <tr valign="top">
                        <th scope="row" class="conversionParameter"><a href="http://flexpaper.com/download/" target="_new">MuPDF-Tools</a> installed?</th>
                        <td><input class="conversionParameter" type="text" name="flexpaper_mudraw_installdir" value="<?php echo $path_to_mudraw ?>" /> <?php if(!$mupdf_installed) { ?><input type="hidden" name="flexpaper_mudraw_enabled" value="false" /><img class="conversionParameter" src="../wp-content/plugins/flexpaper/icon_no.png"><?php }else{ ?><input type="hidden" name="flexpaper_mudraw_enabled" value="true" /><img class="conversionParameter" src="../wp-content/plugins/flexpaper/tick.png"><?php } ?></td>
                    </tr>
